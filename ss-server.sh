#!/bin/bash
# 文件名: ss-service.sh
# 描述:  用于CentOs7 系统，安装ShadowSocks的服务器端。
# 版本:  1.0
# 创建时间:  2016年11月04日
# 修订:  None
...
# 创建链接到/usr/bin
ln -s /usr/bin/Python3/bin/ssserver /usr/bin
ln -s /usr/bin/Python3/bin/sslocal /usr/bin

# 设置开机启动
echo "ssserver -c /etc/shadowsocks/config.json -d start" >> /etc/rc.d/rc.local
